...
with open("predata_v2.txt", "r") as my_train_file:
    data_3 = my_train_file.read().split(',')
...
c = list(zip(seqs, seqs_2, seqs_v2))

shuffle(c)

seqs, seqs_2, seqs_v2 = zip(*c)
...
seqs_t_v2 = torch.Tensor([[one_hot(c) for c in e] for e in seqs_v2])
...
for epoch in range(600): 
    ...
    for i in range(int(len(seqs_t)/mini_batch_size)):
        ...
        images_v2 = seqs_t_v2[i * mini_batch_size : (i+1) * mini_batch_size]
        images_v2 = torch.mean(images_v2.view(-1, 10, 5), dim=1)

        images = images.view(-1, time_step, input_size).to(device)

        images_v2 = images_v2.view(-1, time_step, input_size).to(device)

        images = torch.cat([images[:, :60, :], images_v2[:, :60, :]], 0)

        labels = seq_t[i * mini_batch_size : (i+1) * mini_batch_size].view(-1, 120, 5)

        labels_v2 = torch.flip(labels, dims=[1]) 

        labels = torch.cat([labels[:, :60, :], labels_v2[:, :60, :]], 0)
        ...
        outputs_v2 = torch.argmax(outputs, dim=-1).view(-1, 60) 

        outputs_v2 = torch.cat([outputs_v2[:64,:60], torch.flip(outputs_v2[64:,:60], dims=[1])], dim=-1)
        ...
        labels_v2_1 = seq_t[i * mini_batch_size : (i+1) * mini_batch_size].view(-1, 120, 5)

        labels_v2_1.to(device) 
        ...
        o = 1 - torch.sum(F.one_hot(outputs_v2.to(device), 5).view(-1,120,5) * labels_v2_1.view(-1,120,5).to(device), dim=-1)

        correct = correct + torch.sum(F.one_hot(torch.argmax(outputs, dim=-1), 5) * labels)

        c = torch.sum(F.one_hot(torch.argmax(outputs, dim=-1), 5) * labels, dim=-1).view(-1, 60)
        c = torch.cat([c[:64], c[64:]], 1) 
        c = torch.sum(c, dim=-1) 
    ...
    print("a: ", correct/num)
    print("o: ", collect_o/num_strand_v2)
    print("a_stran: ", num_strand/num_strand_v2)    
    ...
    np.savetxt("weight.txt", o.cpu().numpy()) 
