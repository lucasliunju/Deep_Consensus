...
with open('../data/UnderlyingClusters.txt') as reader, open('training_data_v2.txt', 'w') as writer:
    for index, line in enumerate(reader):
        if index%11 != 0:
            writer.write(line)

with open("training_data_v2.txt", "r") as my_train_file, open("../data/EncodedStrands.txt", "r") as my_train_file_2:
    data_1 = my_train_file.read().splitlines()
    data_2 = my_train_file.read().splitlines()

with open("predata_v2.txt", "w") as file:

    for i in range(len(data_1)):
        data_1[i] = list(data_1[i][::-1])

    file = open("predata_v2.txt", "w")
